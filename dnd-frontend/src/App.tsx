...
const App: React.FC = () => {
    ...
    if (room.is_ai_mode) {
      try {
        const { data, error } = await supabase.functions.invoke('ai-master', { 
          body: { message: content, roomId: room.id, characterContext: character } 
        });

        if (error) {
           console.error('[AI-MASTER Debug]', error);
           let msg = error.message;
           const ctx = (error as any).context;
           
           if (ctx?.json?.error) {
             msg = ctx.json.error;
           } else if (typeof ctx?.text === 'string') {
             msg = `Servidor: ${ctx.text.slice(0, 150)}`;
           } else if (ctx?.status) {
             msg = `Erro HTTP ${ctx.status}: ${error.message}`;
           }

           alert(`O Mestre de RPG teve um contratempo: ${msg}.\n(Referência: ${error.message})`);
        ...
        setRoomMessages(prev => [...prev, aiMessage]);
  ...
  const playNarration = async (text: string, messageId: string) => {
    ...
    try {
      for (const [index, chunk] of chunks.entries()) {
        // ABAIXO: Checagem crítica usando Ref para evitar que loops antigos continuem tocando
        if (currentlyPlayingRef.current !== messageId) break;
        if (isMuted) break;

        const { data, error: ttsError } = await supabase.functions.invoke('tts', { 
          body: { text: chunk, style: room?.style || 'Epic' } 
        });

        if (ttsError) {
          console.error('[tts] Erro no fragmento:', ttsError);
          continue;
        }

        if (data) {
          // Checagem pós-rede para ver se ainda somos o áudio ativo
          if (currentlyPlayingRef.current !== messageId) break;

          // Se 'data' for um Blob, usamos ele. Se for outro formato, tentamos converter.
          const blob = data instanceof Blob ? data : new Blob([data], { type: 'audio/mpeg' });
          
          if (blob.size < 100) {
             console.warn('[tts] Áudio muito curto ou inválido, pulando fragmento.');
             continue;
          }
        ...
        if (index === chunks.length - 1 && currentlyPlayingRef.current === messageId) {
  ...
  const finalizeRoll = () => {
    if (rollingDice.event && channel) {
      ...
      const saveAndTrigger = async (event: DiceEvent) => {
        ...
        await fetchRoomMessages(room!.id);
        if (room!.is_ai_mode) {
          const { data, error } = await supabase.functions.invoke('ai-master', { body: { message: `ROLAGEM: ${content}`, roomId: room!.id, characterContext: character } });
          if (error) {
            console.error('[AI-MASTER Roll Error]', error);
            const ctx = (error as any).context;
            const msg = ctx?.json?.error || ctx?.text?.slice(0, 50) || error.message;
            alert(`Erro na narração da jogada: ${msg}`);
          }
  ...
  const handleGenerateChronicle = async () => {
    ...
    const { data, error } = await supabase.functions.invoke('ai-master', { body: { isChronicle: true, roomId: room.id, characterContext: character } });
    if (error) {
      console.error('[AI-MASTER Chronicle Error]', error);
      const ctx = (error as any).context;
      const msg = ctx?.json?.error || ctx?.text?.slice(0, 50) || error.message;
      alert(`Erro ao gerar crônica: ${msg}`);
    }
    await fetchRoomMessages(room.id);
