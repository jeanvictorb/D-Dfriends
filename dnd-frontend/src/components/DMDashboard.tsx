        ...
        <div className="space-y-6">
          ...
          {pendingProfiles.length > 0 && (
            <div className="panel border-amber-500/30">
              <h3 className="text-xs font-black text-amber-500 uppercase mb-4">Aprovações Pendentes</h3>
              <div className="space-y-2">
                {pendingProfiles.map(p => (
                  <div key={p.id} className="flex items-center justify-between text-white text-xs bg-[#0c1527] p-2 rounded border border-[#2a4387]/50"><span>{p.username}</span><button onClick={() => approvePlayer(p.id)} className="text-green-500"><CheckCircle className="w-4 h-4" /></button></div>
                ))}
              </div>
            </div>
          )}
