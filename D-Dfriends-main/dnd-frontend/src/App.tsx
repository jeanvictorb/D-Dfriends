...
const App: React.FC = () => {
  ...
  const playNarration = async (text: string, messageId: string) => {
    ...
    } catch (e) { console.error(e); setCurrentlyPlaying(null); }
  };
  ...
  if (!character) {
    if (showCreator) return <div className="container mx-auto p-8"><CharacterCreator onCreate={handleCreate} initialClass={tutorialSelectedClass} /><button onClick={() => setShowCreator(false)} className="mt-4 underline text-slate-400">Cancelar</button></div>;
